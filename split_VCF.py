...
def generate_consensus_seq(vcf, ref, chr, start, end, out):
    if chr.startswith("chr"):
        chr = chr.replace("chr", "")
...
def main(boundaries_file, vcf, ref, chr_map, chr, out):
    ...
    logger.info(f"Generating haploblock phased VCFs in {out}")
    for (start, end) in haploblock_boundaries:
        extract_region_from_vcf(vcf, chr, chr_map, start, end, out)
        break
    
    logger.info(f"Generating phased fasta files in {out}")
    for (start, end) in haploblock_boundaries:
        generate_consensus_seq(vcf, ref, chr, start, end, out)
        break
